...
export const OrderFormComponent = (props: OrderFormProps) => {
	...
	const safePrice = React.useMemo(() => (amount && totalPrice / Number(amount)) || priceMarket, [
		totalPrice,
		amount,
		priceMarket,
	]);
	const priceText = React.useMemo(() => formatMessage({ id: 'page.body.trade.header.newOrder.content.price' }), [
		formatMessage,
	]);
	const amountText = React.useMemo(() => formatMessage({ id: 'page.body.trade.header.newOrder.content.amount' }), [
		formatMessage,
	]);
	...
	const changeAmountByButton = React.useCallback(
		(value: number) => {
		...
		<div className={classnames('cr-order-form', className)} onKeyPress={handleEnterPress}>
			<div className="cr-order-item">
				<div className="cr-order-item__dropdown__label">
					{formatMessage({ id: 'page.body.trade.header.newOrder.content.orderType' })}
				</div>
				<DropdownComponent
					list={orderTypes}
			...
			<div className="cr-order-item">
				<div className="cr-order-item__percentage-buttons">
					{AMOUNT_PERCENTAGE_ARRAY.map((value, index) => (
						<PercentageButton value={value} key={index} onClick={changeAmountByButton} />
					))}
				</div>
			</div>
