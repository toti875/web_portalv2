...
const CurrencyInfo: React.FunctionComponent<CurrencyInfoProps> = (props: CurrencyInfoProps) => {
	...
	const childCurrenciesIds = (childCurrencies as unknown as any[]).map(child => child.id);
	const childWallets = wallets.filter(wallet => childCurrenciesIds.includes(wallet.currency)) || {};
	const childBalances = (childWallets as unknown as any[]).map(child => Number(child.balance)) || [];
	const childLockeds = (childWallets as unknown as any[]).map(child => Number(child.locked)) || [];
