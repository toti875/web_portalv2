...
export const TelegramTab: React.FC<TelegramTabProps> = (props: TelegramTabProps) => {
	...
	const dataSource = [
		{
			...
			action: joinGroupClickState ? (
				<Button disabled={joinGroupVerifyState} onClick={handleJoinGroupVerify} type="primary" target="_blank">
					Verify
				</Button>
			) : (
				<Button
					onClick={() => setJoinGroupClickState(true)}
					type="primary"
					href={props.telegramConfig.telegramGroup}
					target="_blank"
				>
					Join
				</Button>
		...
		},
		{
			...
			action: joinChannelClickState ? (
				<Button disabled={joinChannelVerifyState} onClick={handleJoinChannelVerify} type="primary" target="_blank">
					Verify
				</Button>
			) : (
				<Button
					onClick={() => setJoinChannelClickState(true)}
					type="primary"
					href={props.telegramConfig.telegramChannel}
					target="_blank"
				>
					Join
				</Button>
		...
		},
		{
			key: '3',
			step: '3',
			task: 'Enter Telegram Username Joined. Ex: @Fortem',
			action: (
				<Search
	...
	return <div>{usernameState !== '' ? resultView : tableView}</div>;
};
